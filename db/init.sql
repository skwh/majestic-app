...
AS $canary_make_sensor_color$
  ...
  BEGIN
    ...
    idx := floor((random() * cardinality(colors))) :: int;
    WHILE (SELECT EXISTS(SELECT 1 FROM canary_recent_data WHERE sensor_color=colors[idx])) LOOP
      idx := floor((random() * cardinality(colors)));
    END LOOP;
    RETURN colors[idx];
  END;
